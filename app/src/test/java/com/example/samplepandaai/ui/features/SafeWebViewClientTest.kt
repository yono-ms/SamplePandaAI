...
class SafeWebViewClientTest {
    ...
    private lateinit var isSafeDomainUseCase: IsSafeDomainUseCase
    private lateinit var client: SafeWebViewClient

    @Before
    fun setUp() {
        context = mockk(relaxed = true)
        isSafeDomainUseCase = mockk()
        client = SafeWebViewClient(context, isSafeDomainUseCase)
    ...
    fun `handleUrl returns false when UseCase returns true`() {
        ...
        every { isSafeDomainUseCase(urlStr) } returns true
    ...
    fun `handleUrl returns true and starts activity when UseCase returns false`() {
        ...
        every { Uri.parse(urlStr) } returns mockUri
        every { mockUri.toString() } returns urlStr
        every { isSafeDomainUseCase(urlStr) } returns false
