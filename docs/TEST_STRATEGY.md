...
| S-6 | TopAppBar に戻るボタンが表示され、クリック可能か                 | `RepoListScreenTest`        |   ✅   |
...
| I-3 | 画面遷移後に戻るボタンで前の画面に正しく復帰できるか                | `UserNameIntegrationTest`   |   ✅   |
...
| I18N-6 | **アクセシビリティ**: 拡大フォント設定時でもテキストが適切に表示されるか                                | `Preview` (fontScale) による視覚確認  |   ✅   |
...
| ID   | テスト観点                                         | 検証手法 / 対応状況           | ステータス |
|:-----|:----------------------------------------------|:----------------------|:-----:|
| CI-1 | プルリクエスト作成時に自動的にビルドが実行され、成否が判定されるか             | GitHub Actions (構築予定) |   ⏳   |
| CI-2 | プルリクエスト作成時にユニットテストが自動実行され、すべてパスするか            | GitHub Actions (構築予定) |   ⏳   |
| CI-3 | ビルドまたはテストが失敗した場合に、PR のマージがブロックされるか (GitHub設定) | GitHub リポジトリ設定 (導入予定) |   ⏳   |

## 4. 今後の課題・不足しているテスト

1. **CI 環境の構築 (最優先)**:
    - GitHub Actions による自動ビルド・ユニットテストのパイプライン構築（次タスク）。
2. **インストゥルメントテストの CI 実行**:
