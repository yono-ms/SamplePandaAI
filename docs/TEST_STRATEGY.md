...
| U-4 | 与えられた URL が許可されたドメインか判定できるか  | `IsSafeDomainUseCaseTest` (更新)      |  🔄   |
...
| S-8  | WebView 内での URL 遷移がドメインに応じて適切に制御されるか          | `SafeWebViewClientTest` (更新予定)                         |  🔄   |
| S-9  | 詳細画面のタイトルが正しく表示されるか                           | `RepoDetailScreenTest`, `LicenseDetailScreenTest` (新規) |  🔄   |
| S-10 | ライセンス一覧で項目をタップした際、正しいURLが遷移先に渡されるか            | `LicenseScreenTest` (更新予定)                             |  🔄   |
